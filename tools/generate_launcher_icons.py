...
for density,size in [('ldpi',36),('mdpi',48),('hdpi',72),('xhdpi',96),('xxhdpi',144),('xxxhdpi',192)]:
    for suffix,source in [('',SOURCE),('_round',SOURCE.replace('rx="23"','rx="54"'))]:
        save(RES/f'mipmap-{density}/an_paint_launcher{suffix}.png',cairosvg.svg2png(bytestring=source.encode(),output_width=size,output_height=size))
...
foreground+=f'    <path android:fillColor="#FFFFFF" android:fillType="evenOdd" android:pathData="{paths[0].get("d")}"/>\n'
...
for api in [26,33]:
    ...
    if api>=33: xml+='    <monochrome android:drawable="@drawable/an_paint_launcher_monochrome"/>\n'
    xml+='</adaptive-icon>\n'
