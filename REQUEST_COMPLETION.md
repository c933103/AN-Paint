# Request completion audit — 0.0.23
...
|---|---|
| Copyleft icons; distinct app identity | KDE Breeze LGPL icons with original SVGs and generated hashes. Local.20 replaces the old monogram with an original AGPL paintbrush/stroke launcher. Package `paint.anpaint.android` installs separately from prior builds; copyright and source are under File > About / licences / credits. |
...
| Tool grouping and orientation | Main/File/Edit/View/Color tabs remain visible in both orientations; each opens its panel above the full-width canvas. Navigate is first/default, Drawing replaces Brush as the group name, and every tool has a visible label. Collapse sits on the tabs row. |
| Requested drawing/selection additions | Watercolor; Heart/Star/Arrow; polygon double-tap completion; rounded-corner radius; corner/edge resize and rotation for rectangle and free-form masks; Select all in Edit; Cut/Copy/Paste beside Undo/Redo. |
| Navigation | Pinch/pan, cursor drawing, magnified preview, 100% zoom and Fit. A shared clamp/thumb/drag model reaches both pan limits, including trim bounds and cursor drawing. Resizing preserves image-space centre; fitted views refit and legacy offscreen drafts recover. Local.19 also autosaves a magnification-only setting change while the editor stays open. |
...
| Colours | Color tab contains FG/BG, Swap, Reset B/W, standard/recent/saved palettes. Every selector previews FG/BG live; Use color commits, Cancel restores. Add to palette is optional and adds to the same saved colors shown in Color; explicit palette management supports replacement/removal. RGB/HSV/HSL numeric entry uses the selected locale. |
...
| Formats and export | Separate Save as and Export as panels preserve exact canvas pixels or create lossy/reduced output respectively. Save reuses the successful Save as URI/name/format and persists its permission. DIB/TIFF/ICO/Base64 open and save; ASCII exports text; AVIF already opens/saves with quality or lossless mode. PDF opens as a raster image. PDF/TIFF show a page selector; the selected page stays with assembly sources. Animated GIF/APNG/WebP warn before still import, including Base64 and assembly. Every export keeps the full drawing unsaved and leaves its Save destination intact. See CODEC_SUPPORT.md for supported variants, limits and remaining SVG/layered-format candidates. |
...
| Font and licence UI | Actual-font previews, 20 named choices, eleven bundled OFL fonts, complete credits and fixed licence footer with Copy all. Dubai/STC review examines exact binaries, public terms and upstream introduction history; their redistribution rights for AN Paint remain unestablished. |
| App language and inherited translations | View > Languages offers 68 locale variants (including English), plus Use device language. All 73 upstream translation source files are verified against pinned Git blobs; 42 equivalent common terms are mapped into new controls. Variants with no mapped translations are excluded. Android 13+ uses the same app-language setting as Android; older versions retain the preference internally. Partial translations fall back to English. |
| Vertical text and new languages | Literary Chinese and traditional-script Mongolian foundations; vertical ribbon labels. Inserted text supports right-to-left or left-to-right columns, mixed upright CJK and sideways alphabetic runs, fully sideways runs, or upright Unicode clusters. Preview and output share shaping; text settings persist. Native Android dialogs retain platform layout. |
...
0.0.23 also audits action translations across locales using pinned AOSP terminology and reviewed editor-specific corrections. The unsaved prompt cannot display the same label for Discard changes and Keep editing. See translations/ACTION-REVIEW.md.
