...
class WorkspaceRefinementTest {
    ...
    @Test fun colourArrowStartsExpandedStaysPinnedAndRemembersCollapse() {
        ...
        click("colour_status");assertFalse(bar.isShown);assertTrue(canvas.height>height)
        assertEquals("Collapsing colours must not move the indicator",pinnedBounds,screenBounds(indicator))
        assertEquals(pinnedBounds.bottom,screenBounds(canvas).bottom)
    ...
    @Test fun wholeSidebarCollapsesBelowToolbarAndSurvivesRotation() {
        ...
        val width=canvas.width;click("sidebar_toggle");assertFalse(toggle.isSelected)
        assertEquals(View.GONE,view<View>("sidebar").visibility);assertTrue(canvas.width>width)
