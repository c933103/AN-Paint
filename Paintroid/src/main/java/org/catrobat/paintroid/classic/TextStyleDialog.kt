...
class TextStyleDialog(private val activity: Activity,private val initial: TextSettings,private val foreground: Int,private val background: Int,
    ...
    fun show(): AlertDialog {
        ...
        val input=EditText(activity).apply { tag="text_content";hint="Type text";setText(initial.text);inputType=InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_MULTI_LINE;minLines=2 }
        body.addView(input)
        val catalog=FontCatalog(activity)
        val font=Spinner(activity).apply { tag="text_font";contentDescription="Font family";adapter=catalog.adapter();setSelection(initial.font.coerceIn(catalog.fonts.indices)) }
        body.addView(label("Font"));body.addView(font,LinearLayout.LayoutParams(-1,dp(48)))
        ...
        listOf("Size (pixels)" to size,"Line spacing (%)" to spacing).forEach { (name,field) ->
        ...
        val bold=check("Bold","text_bold",initial.bold);val italic=check("Italic","text_italic",initial.italic)
        val underline=check("Underline","text_underline",initial.underline);val strike=check("Strikethrough","text_strike",initial.strike)
        ...
        val align=Spinner(activity).apply { tag="text_alignment";contentDescription="Text alignment";adapter=ArrayAdapter(activity,android.R.layout.simple_spinner_dropdown_item,arrayOf("Left aligned","Centred","Right aligned"));setSelection(initial.alignment) }
        body.addView(align,LinearLayout.LayoutParams(-1,dp(48)))
        val box=check("Fill text box with BG colour","text_box",initial.box);body.addView(box)
        body.addView(label("Preview"))
        ...
        val credit=Button(activity).apply { text="Font licences";tag="font_licences";isAllCaps=false;setOnClickListener { LegalInfo.showAsset(activity,"Font licences","legal/FONT_NOTICES.txt") } }
        ...
        fun update() {
            ...
            preview.text=input.text.toString().ifBlank { "Aa Bb 0123 · あいう" }
            preview.setTextSize(TypedValue.COMPLEX_UNIT_PX,(size.text.toString().toFloatOrNull() ?: 32f).coerceIn(16f,dp(64).toFloat()))
            ...
            preview.setLineSpacing(0f,((spacing.text.toString().toFloatOrNull() ?: 100f)/100).coerceIn(.5f,3f))
        }
        ...
        val dialog=AlertDialog.Builder(activity).setTitle("Place text").setView(ScrollView(activity).apply { addView(body) })
            .setNegativeButton("Cancel",null).setPositiveButton("Add text",null).create()
        dialog.setOnShowListener { dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            val pixels=size.text.toString().toFloatOrNull();val percent=spacing.text.toString().toFloatOrNull()
            when {
                pixels==null || pixels !in 1f..1024f -> size.error="Use 1–1024 pixels."
                percent==null || percent !in 50f..300f -> spacing.error="Use 50–300%."
                else -> {
